...
def handler(job):
    ...
    try:
        # 🔧 FIX: Safely check for test request
        ...
        mode = job_input.get("mode", "replace")
        ...
        try:
            if check_coord is None:
                if mode == "animate":
                    workflow_file = '/newWanAnimate_noSAM_animate_api.json'
                else:
                    workflow_file = '/newWanAnimate_noSAM_api.json'
            else:
                if mode == "animate":
                    workflow_file = '/newWanAnimate_point_animate_api.json'
                else:
                    workflow_file = '/newWanAnimate_point_api.json'
            
            # 🔧 WORKFLOW VALIDATION: Check if workflow file exists
            if not os.path.exists(workflow_file):
                logger.error(f"❌ Workflow file not found: {workflow_file}")
                return {"error": f"Workflow file not found: {workflow_file}", "status": "failed"}
            ...
            return {"error": f"Workflow loading failed: {str(e)}", "status": "failed"}
        ...
        try:
            ...
            # 🔧 OPTIONAL PARAMETERS: SAM2 coordinates (only set if provided)
            if check_coord is not None and "107" in prompt and "inputs" in prompt["107"]:
            ...
            return {"error": f"Workflow configuration failed: {str(e)}", "status": "failed"}
        
        logger.info(f"🎬 Processing: {width}x{height}, fps={fps}, steps={steps}, mode={mode}")
        ...
        if output_format not in ["minio", "base64"]:
            logger.warning(f"⚠️ Invalid output_format '{output_format}', defaulting to 'minio'")
            output_format = "minio"
        
        logger.info(f"📤 Output format: {output_format}")

        # 🔧 PRIORITIZED VIDEO SELECTION
        priority_nodes = ["30", "194", "182", "164", "155"]
        selected_video = None
        selected_node = None
        ...
        if output_format == "base64":
            ...
            return {
                ...
                "video": selected_video,  # 호환성을 위해 video 필드도 유지
